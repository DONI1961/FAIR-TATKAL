...
export const TrainAnimation = () => {
  return (
    <div className="relative h-64 w-full overflow-hidden">
      {/* Railway Track */}
      <div className="absolute bottom-10 h-1 w-full bg-gradient-to-r from-transparent via-foreground/10 to-transparent" />
      ...
      >
        {/* Engine */}
        <div className="relative h-12 w-40 rounded-r-full bg-gradient-to-r from-slate-700 to-slate-400 dark:from-slate-600 dark:to-slate-800 shadow-lg dark:shadow-[0_0_20px_rgba(255,255,255,0.1)]">
          {/* Windows */}
        ...
        {[1, 2, 3].map((i) => (
          <div key={i} className="ml-1 h-10 w-32 rounded-sm bg-gradient-to-b from-slate-600 to-slate-800 border-t border-white/10">
            <div className="mt-2 flex gap-2 px-2">
              {[1, 2, 3, 4].map((j) => (
                <div key={j} className="h-2 w-4 rounded-sm bg-blue-400/30" />
              ))}
            </div>
          </div>
        ))}
      ...
      {[1, 2, 3].map((i) => (
        <motion.div
          ...
          className="absolute h-[1px] w-40 bg-foreground/5"
          style={{ top: `${20 + i * 15}%` }}
        />
      ))}
    </div>
